...
<main class="flex-1 bg-[#FAFAFA] overflow-y-auto px-4 lg:px-10 py-6 lg:py-8 w-full relative">
    ...
    <!-- Filter Bar -->
    <form action="{{ route('admin.transaction.index') }}" method="GET" class="bg-white p-4 rounded-t-3xl border border-gray-100 border-b-0 flex flex-col md:flex-row gap-4">
        <!-- Search -->
        <div class="relative w-full md:w-1/3">
            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </div>
            <input type="text" name="search" value="{{ request('search') }}" class="block w-full pl-11 pr-3 py-3 border border-gray-200 rounded-xl leading-5 bg-gray-50 placeholder-gray-400 focus:outline-none focus:bg-white focus:border-[#CC9863] focus:ring-1 focus:ring-[#CC9863] text-sm font-semibold transition" placeholder="Cari No. Invoice atau Pelanggan..." onblur="this.form.submit()">
        </div>

        <!-- Dropdown Filters -->
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 w-full md:w-2/3">
            <select name="cabang" onchange="this.form.submit()" class="block w-full pl-3 pr-8 py-3 text-sm font-semibold border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-1 focus:ring-[#CC9863] focus:border-[#CC9863]">
            ...
            <select name="tanggal" onchange="this.form.submit()" class="block w-full pl-3 pr-8 py-3 text-sm font-semibold border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-1 focus:ring-[#CC9863] focus:border-[#CC9863]">
            ...
            <select name="metode" onchange="this.form.submit()" class="block w-full pl-3 pr-8 py-3 text-sm font-semibold border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-1 focus:ring-[#CC9863] focus:border-[#CC9863]">
            ...
            @if(request()->anyFilled(['search', 'cabang', 'tanggal', 'metode']) && request('cabang') !== 'all')
                <a href="{{ route('admin.transaction.index') }}" class="flex items-center justify-center bg-red-50 text-red-600 rounded-xl text-sm font-bold hover:bg-red-100 transition">Reset</a>
            @endif
        </div>
    </form>
            ...
            <table class="w-full whitespace-nowrap">
                <thead class="bg-gray-50 text-gray-500 text-xs font-extrabold uppercase tracking-wider text-left border-y border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center">Aksi (CUD)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                    ...
                    @if(isset($transactions))
                        @forelse($transactions as $trx)
                            ...
                            @endphp
                            <tr class="hover:bg-gray-50/50 transition-colors {{ $belumLunas ? 'bg-red-50/20' : '' }}">
                                <td class="px-6 py-4">
                                    <p class="font-extrabold text-gray-900">{{ $trx->nomor_nota }}</p>
                                    <p class="text-[11px] font-semibold text-gray-400">{{ \Carbon\Carbon::parse($trx->tanggal_waktu)->format('d M Y, H:i') }} WIB</p>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="font-bold text-gray-700">{{ $trx->branch->nama_cabang ?? 'Outlet Pusat' }}</p>
                                    <p class="text-[11px] font-semibold text-gray-500 flex items-center gap-1 mt-0.5">
                                ...
                                <td class="px-6 py-4">
                                    @if($trx->details->count() > 0)
                                        ...
                                        @endphp
                                        <p class="text-sm font-bold text-gray-800">{{ $firstItem->qty }}x {{ $firstItem->variant->nama_varian ?? 'Produk' }}</p>
                                        @if($moreCount > 0)
                                            <p class="text-[11px] font-semibold text-[#CC9863] mt-0.5">+{{ $moreCount }} item lainnya</p>
                                        @endif
                                    @else
                                    ...
                                    @if($trx->member)
                                        <p class="text-[10px] font-bold text-blue-600 flex items-center gap-1 mt-1 bg-blue-50 w-fit px-1.5 py-0.5 rounded">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path></svg>
                                            Plg: {{ $trx->member->nama }}
                                        </p>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    @else
                                        <span class="text-[10px] text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-md inline-block mt-1 border border-green-100">Lunas</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
    ...
    <a href="{{ route('admin.transaction.edit', $trx->id) }}" class="text-green-600 bg-green-50 hover:bg-green-100 hover:shadow-sm px-3 py-1.5 rounded-lg text-[11px] font-extrabold transition border border-green-200 flex items-center gap-1.5">
        ...
        Lanjut Edit
                                        ...
                                        @elseif($trx->approval_status === 'pending_edit' || $trx->approval_status === 'pending_delete')
                                            ...
                                            <span class="text-orange-500 bg-orange-50 px-3 py-1.5 rounded-lg text-[10px] font-extrabold border border-orange-100">
                                                Menunggu Owner
                                            </span>
                                        @else
                                            <!-- Keadaan Normal Terkunci -->
                                            <button onclick="openRequestModal('{{ $trx->id }}', 'edit')" class="text-blue-500 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg text-[11px] font-extrabold transition border border-blue-100">
                                                Ajukan Edit
                                            </button>
                                            <button onclick="openRequestModal('{{ $trx->id }}', 'delete')" class="text-red-500 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg text-[11px] font-extrabold transition border border-red-100">
                                                Ajukan Hapus
                                            </button>
                                        @endif
                                    </div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            <span class="text-sm font-semibold text-gray-500">Menampilkan {{ $transactions->firstItem() }} - {{ $transactions->lastItem() }} dari {{ $transactions->total() }}</span>
            <div>
    ...
    <div id="requestModal" class="fixed inset-0 z-[100] hidden bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform scale-95 transition-transform duration-300" id="requestCard">
            <div class="p-6 bg-orange-50 border-b border-orange-100 flex items-center gap-4">
                <div class="w-12 h-12 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center shadow-inner shrink-0">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-600 mt-0.5 font-medium">Sertakan alasan yang jelas untuk mempercepat persetujuan Owner.</p>
                </div>
            </div>
            <div class="p-6">
                ...
                <div class="mb-5">
                    <label class="block text-sm font-extrabold text-gray-800 mb-2">Alasan Pengajuan <span class="text-red-500">*</span></label>
                    <textarea id="requestReason" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50 font-medium focus:outline-none focus:border-orange-500 focus:bg-white transition" placeholder="Contoh: Kasir salah input metode pembayaran..."></textarea>
                    <p id="requestErrorMsg" class="text-xs text-red-500 mt-2 font-bold hidden"></p>
                </div>

                <div class="flex gap-3">
                    <button onclick="closeRequestModal()" class="flex-1 px-4 py-3 bg-white border border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition">Batal</button>
                    <button onclick="submitRequest()" id="btnSubmitRequest" class="flex-1 px-4 py-3 bg-[#CC9863] text-white font-bold rounded-xl hover:bg-[#b58555] shadow-lg shadow-[#CC9863]/30 transition flex items-center justify-center gap-2">
                        Kirim Pengajuan
                    </button>
                </div>
            </div>
    ...
    function submitRequest() {
        ...
        .then(data => {
            if(data.success) {
                ...
                location.reload(); // Refresh halaman agar tombol berubah menjadi "Menunggu Owner"
            } else {
