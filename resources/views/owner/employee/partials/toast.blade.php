<div id="employee-toast" class="fixed right-4 top-4 z-50 hidden w-[min(24rem,calc(100vw-2rem))]" role="status" aria-live="polite">
    <div class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-4 shadow-xl shadow-gray-900/10">
        <div class="flex items-start gap-3">
            <div id="employee-toast-icon" class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-green-100 text-green-600">
            ...
            <div class="min-w-0 flex-1">
                <p id="employee-toast-title" class="text-sm font-bold text-gray-900">Berhasil</p>
                <p id="employee-toast-message" class="mt-1 text-sm font-semibold text-gray-700"></p>
                <div id="employee-toast-actions" class="mt-3 hidden gap-2">
                    <button type="button" id="employee-toast-cancel" class="rounded-lg border border-gray-200 px-3 py-2 text-xs font-bold text-gray-600 hover:bg-gray-50">Batal</button>
                    <button type="button" id="employee-toast-confirm" class="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white hover:bg-red-700">Lanjutkan</button>
                </div>
            </div>
            <button type="button" id="employee-toast-close" class="text-gray-400 hover:text-gray-600" aria-label="Tutup notifikasi">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const toast = document.getElementById('employee-toast');
        const icon = document.getElementById('employee-toast-icon');
        const title = document.getElementById('employee-toast-title');
        const message = document.getElementById('employee-toast-message');
        const actions = document.getElementById('employee-toast-actions');
        const close = document.getElementById('employee-toast-close');
        const cancel = document.getElementById('employee-toast-cancel');
        const confirm = document.getElementById('employee-toast-confirm');
        ...
        document.addEventListener('submit', function (event) {
            const form = event.target.closest('[data-employee-action]');
            if (!form) {
                return;
            }
            ...
            pendingForm = form;
            const isRestore = form.dataset.employeeAction === 'restore';
            showToast(
                isRestore ? 'Konfirmasi aktivasi' : 'Konfirmasi penonaktifan',
                isRestore ? 'Pegawai ini akan diaktifkan kembali.' : 'Pegawai ini tidak dapat login setelah dinonaktifkan.',
                true,
                isRestore,
            );
        });
