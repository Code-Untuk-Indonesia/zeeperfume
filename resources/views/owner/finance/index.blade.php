...
@section('content')
    <main class="flex-1 bg-[#F3F4F6] overflow-y-auto px-4 lg:px-8 py-6 lg:py-8 w-full relative">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">Analytics</h1>
                <p class="text-gray-500 text-sm mt-1 font-medium">Detailed overview of your financial situation</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="lg:col-span-2 bg-white rounded-[24px] border border-gray-100 shadow-sm p-5 sm:p-6 flex flex-col">
                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                    <div>
                        <p class="text-[10px] font-extrabold text-indigo-500 uppercase tracking-wider mb-1">Financial
                            Performance</p>
                        <h2 class="text-base sm:text-lg font-extrabold text-gray-900">Income & Expense Overview</h2>
                        <p class="text-[11px] sm:text-xs text-gray-400 font-medium mt-1">Pergerakan omzet dan biaya
                            {{ $monthName }}.</p>
                    </div>
                    <div class="flex items-center gap-4 text-[10px] font-bold">
                        <div class="flex items-center gap-2"><span
                                class="w-2.5 h-2.5 rounded-full bg-indigo-500"></span><span
                                class="text-gray-500">Omzet</span></div>
                        <div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-gray-400"></span><span
                                class="text-gray-500">Total Biaya</span></div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 mt-5 pt-5 border-t border-gray-100">
                    <div class="bg-indigo-50/70 rounded-2xl px-4 py-3">
                        <p class="text-[9px] uppercase tracking-wide font-extrabold text-indigo-400">Total Omzet</p>
                        <p class="text-sm sm:text-base font-black text-indigo-700 mt-1">Rp
                            {{ number_format($totalOmzet, 0, ',', '.') }}</p>
                    </div>
                    <div class="bg-gray-50 rounded-2xl px-4 py-3">
                        <p class="text-[9px] uppercase tracking-wide font-extrabold text-gray-400">Total Beban</p>
                        <p class="text-sm sm:text-base font-black text-gray-700 mt-1">Rp
                            {{ number_format($totalBeban, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[24px] border border-gray-100 shadow-sm p-5 sm:p-6 flex flex-col">
                <div class="mb-5">
                    <p class="text-[10px] font-extrabold text-[#CC9863] uppercase tracking-wider mb-1">Cost Structure</p>
                    <h2 class="text-base font-extrabold text-gray-900">Expense Breakdown</h2>
                ...
                <div class="flex-1 flex items-center justify-center py-2">
                    <div class="relative w-[190px] h-[190px] sm:w-[210px] sm:h-[210px]">
                        ...
                        <div
                            ...
                            <span class="text-[9px] uppercase tracking-wider font-bold text-gray-400">Total Beban</span>
                ...
                <div class="space-y-3 mt-4">
                    <div class="p-3.5 bg-gray-50 rounded-2xl">
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2"><span
                                    class="w-2.5 h-2.5 bg-gray-800 rounded-full shrink-0"></span><span
                                    class="text-xs font-bold text-gray-600">Modal / HPP</span></div>
                        ...
                        <p class="text-[10px] text-gray-400 font-semibold mt-2">Rp
                            {{ number_format($totalHpp, 0, ',', '.') }}</p>
                    </div>
                    <div class="p-3.5 bg-indigo-50/60 rounded-2xl">
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2"><span
                                    class="w-2.5 h-2.5 bg-indigo-500 rounded-full shrink-0"></span><span
                                    class="text-xs font-bold text-gray-600">Operasional</span></div>
        ...
        <!-- ================= TABEL RINCIAN MODAL BARANG (HPP) ================= -->
        <div class="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden mb-6">
            <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-bold text-gray-900">Rincian Harga Pokok Penjualan (HPP)</h2>
                    <p class="text-[11px] text-gray-500 font-semibold mt-1">Daftar modal produk terjual yang ditarik dari
                        harga modal stok di bulan {{ $monthName }}.</p>
                </div>
            </div>
                ...
                <table class="w-full text-left whitespace-nowrap">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center">Terjual (Qty)</th>
                            <th class="px-6 py-4 text-right">Harga Modal Satuan</th>
                            <th class="px-6 py-4 text-right">Total Modal (HPP)</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-gray-700 divide-y divide-gray-50">
                        @forelse($hppDetails as $item)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    <p class="font-bold text-gray-900">
                                        {{ $item->variant->product->nama_produk ?? 'Unknown' }}</p>
                                    <span
                                        class="inline-block mt-1 text-[10px] font-bold text-gray-500">{{ $item->variant->nama_varian ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="font-bold text-gray-800">{{ $item->total_qty }}</span> <span
                                        class="text-[10px] font-medium text-gray-400">{{ strtolower($item->variant->satuan ?? 'pcs') }}</span>
                                </td>
                                <td class="px-6 py-4 text-right text-gray-500 font-semibold">
                                    Rp {{ number_format($item->modal_satuan, 0, ',', '.') }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-gray-400 font-medium italic">Belum
                                    ada barang terjual atau modal tercatat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if ($totalHpp > 0)
                        <tfoot class="bg-gray-50 text-gray-900 border-t border-gray-200">
                            <tr>
                                <td colspan="3"
                                    class="px-6 py-5 font-black tracking-widest text-xs uppercase text-right">Total
                                    Keseluruhan HPP</td>
                                <td class="px-6 py-5 text-right font-black">Rp {{ number_format($totalHpp, 0, ',', '.') }}
                                </td>
                            </tr>
                        </tfoot>
                    @endif
        ...
        <div class="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden mb-6">
            <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-bold text-gray-900">Daftar Pengeluaran Operasional</h2>
                    <p class="text-[11px] text-gray-500 font-semibold mt-1">Catatan pengeluaran selama
                        {{ $monthName }}.</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                        ...
                        @forelse($expenses as $exp)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <span
                                        class="inline-block mt-1 text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $exp->kategori_pengeluaran }}</span>
                                </td>
                                <td class="px-6 py-4 font-medium text-gray-700">{{ $exp->branch->nama_cabang ?? 'Pusat' }}
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-6 h-6 rounded-full bg-[#CC9863] text-white flex items-center justify-center text-[10px] font-bold uppercase shrink-0">
                                            {{ substr($exp->user->nama_lengkap ?? 'A', 0, 1) }}
                                        </div>
                                        <p class="font-bold text-gray-800 text-xs">
                                            {{ explode(' ', $exp->user->nama_lengkap ?? 'Unknown')[0] }}</p>
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center text-gray-400 font-medium italic">Tidak
                                    ada catatan pengeluaran operasional bulan ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="bg-white rounded-[24px] border border-gray-100 shadow-sm overflow-hidden mb-6">
            <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                <div>
                    <h2 class="text-base font-bold text-gray-900">Rincian Keuangan per Cabang</h2>
                    <p class="text-[11px] text-gray-500 font-semibold mt-1">Sistem Perhitungan: Laba Bersih = Omzet - (HPP
                        + Ops) per cabang.</p>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left whitespace-nowrap">
                    <thead
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-right text-indigo-500">Omzet (Kotor)</th>
                            <th class="px-6 py-4 text-right">Modal / HPP</th>
                            <th class="px-6 py-4 text-right text-blue-500">Laba Kotor</th>
                            <th class="px-6 py-4 text-right">Pengeluaran Ops</th>
                            <th class="px-6 py-4 text-right text-green-600">Laba Bersih</th>
                            <th class="px-6 py-4 text-center">Margin Bersih</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-gray-700 divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50 transition cursor-pointer">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-[10px] uppercase">
                                ...
                                <td class="px-6 py-4 text-right font-bold text-gray-900">Rp
                                    {{ number_format($report->omzet, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-right text-gray-500 font-semibold">Rp
                                    {{ number_format($report->hpp, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-right font-black text-gray-900">Rp
                                    {{ number_format($report->laba_kotor, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-right text-red-500 font-semibold">Rp
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="{{ $report->margin >= 0 ? 'bg-indigo-50 text-indigo-700' : 'bg-red-50 text-red-700' }} text-[10px] font-extrabold px-2.5 py-1 rounded-md">
                                        {{ $report->margin }}%
                                    </span>
                                </td>
                            </tr>
                    ...
                    <tfoot class="bg-gray-50 text-gray-900 border-t border-gray-200">
                        <tr>
                            <td class="px-6 py-5 font-black tracking-widest text-xs uppercase">Total {{ $monthName }}
                            </td>
                            <td class="px-6 py-5 text-right font-black">Rp {{ number_format($totalOmzet, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-5 text-right font-bold text-gray-500">Rp
                                {{ number_format($totalHpp, 0, ',', '.') }}</td>
                            <td class="px-6 py-5 text-right font-black">Rp {{ number_format($labaKotor, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-5 text-right font-bold text-red-500">Rp
                                {{ number_format($totalPengeluaran, 0, ',', '.') }}</td>
                            <td
                                class="px-6 py-5 text-right font-black {{ $labaBersih >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                Rp {{ number_format($labaBersih, 0, ',', '.') }}</td>
                            <td class="px-6 py-5 text-center font-black">{{ $marginPercentage }}%</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        (function() {
            ...
            function renderFinanceCharts() {
                ...
                if (expenseCanvas) {
                    ...
                    const totalPengeluaran = Number(financeData.totalPengeluaran || 0);
                    const hasExpense = totalHpp > 0 || totalPengeluaran > 0;
                    const expenseData = hasExpense ? [totalHpp, totalPengeluaran] : [1, 0];
                    ...
                    new Chart(expenseCanvas, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                data: expenseData,
                                backgroundColor: hasExpense ? ['#1F2937', '#6366F1'] : ['#E5E7EB',
                                    '#F3F4F6'
                                ],
                                borderWidth: 0,
                                ...
                                tooltip: {
                                    ...
                                    callbacks: {
                                        label(context) {
                                            ...
                                            const total = totalHpp + totalPengeluaran;
                                            const percentage = total > 0 ? Math.round((value / total) * 100) :
                                            0;
                                            return ' ' + context.label + ': ' + rupiah(value) + ' (' +
                                                percentage + '%)';
                                        }
                                    }
                                }
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', renderFinanceCharts);
            } else {
                renderFinanceCharts();
            }
        })();
