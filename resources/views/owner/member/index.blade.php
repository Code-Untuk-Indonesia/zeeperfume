...
<main class="flex-1 bg-[#FAFAFA] overflow-y-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-8 w-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 lg:mb-8">
        ...
        <a href="{{ route('owner.member.create') }}" class="w-full md:w-auto bg-[#CC9863] text-white px-5 py-3 rounded-xl font-bold shadow-sm hover:bg-[#b58555] transition flex items-center justify-center gap-2 text-sm shrink-0">
    ...
    <form method="GET" action="{{ route('owner.member.index') }}" class="bg-white p-4 lg:p-5 rounded-t-3xl border border-gray-100 border-b-0 flex flex-col md:flex-row gap-3 justify-between">
        ...
        <div class="flex gap-2">
            ...
            @if (request()->hasAny(['search', 'tipe']))
                <a href="{{ route('owner.member.index') }}" class="px-5 py-3 rounded-xl border border-gray-200 text-gray-600 text-sm font-bold hover:bg-gray-50">Reset</a>
            @endif
        </div>
    </form>
