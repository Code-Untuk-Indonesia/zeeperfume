    ...
    <main class="flex-1 bg-[#FAFAFA] overflow-y-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-8 w-full relative">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 lg:mb-8">
            <div>
                ...
                <p class="text-gray-500 text-xs sm:text-sm mt-1">Kelola data pelanggan setia, klasifikasi member, dan riwayat
                    belanja.</p>
            </div>
            <a href="{{ route('owner.member.create') }}"
                class="w-full md:w-auto bg-[#CC9863] text-white px-5 py-3 rounded-xl font-bold shadow-sm hover:bg-[#b58555] transition flex items-center justify-center gap-2 text-sm shrink-0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                    ...
                    <h4 class="text-base lg:text-lg font-extrabold text-gray-900">{{ number_format($totalMembers) }} Orang
                    </h4>
                </div>
            </div>
            <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                    ...
                    <h4 class="text-base lg:text-lg font-extrabold text-gray-900">{{ number_format($vipMembers) }} Orang
                    </h4>
                </div>
            </div>
            <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                ...
                <div>
                    ...
                    <h4 class="text-base lg:text-lg font-extrabold text-gray-900">Rp
                        {{ formatSingkatan($totalBelanjaSemua) }}</h4>
                </div>
            </div>
        </div>
        ...
        <!-- Filter Bar Form -->
        <form id="filter-form"
            class="bg-white p-4 lg:p-5 rounded-t-3xl border border-gray-100 border-b-0 flex flex-col xl:flex-row gap-4 justify-between items-center">
            <!-- Search -->
            <div class="relative w-full xl:w-96 shrink-0">
                ...
                <input type="text" id="search-input" name="search" value="{{ request('search') }}"
                    class="block w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:bg-white focus:border-[#CC9863] focus:ring-2 focus:ring-[#CC9863]/30 text-sm transition"
                    placeholder="Cari nama, no HP, atau ID...">
            </div>

            <!-- Dropdown Filters -->
            <div class="grid grid-cols-1 md:flex gap-3 w-full xl:w-auto">
                <select id="tipe-filter" name="tipe"
                    class="block w-full pl-3 pr-8 py-3 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:border-[#CC9863] focus:bg-white transition cursor-pointer appearance-none">
        ...
        <div class="bg-white rounded-b-3xl border border-gray-100 shadow-sm overflow-hidden mb-6 relative">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left whitespace-nowrap">
                    <thead
                        class="bg-gray-50 text-gray-400 text-[10px] md:text-xs font-bold uppercase tracking-wider border-y border-gray-100">
                        <tr>
            ...
            <div id="pagination-container"
                class="px-4 md:px-6 py-4 border-t border-gray-100 flex justify-center sm:justify-start">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function getFilterUrl() {
                ...
                if (tipeFilter.value && tipeFilter.value !== 'semua') url.searchParams.append('tipe', tipeFilter
                    .value);
                return url.toString();
            }
