                ...
                <table class="w-full text-left whitespace-nowrap">
                    ...
                    <tbody id="table-body" class="divide-y divide-gray-100 text-sm text-gray-700">
                        ...
                        @forelse($outlets as $outlet)
                            <tr class="hover:bg-gray-50 transition {{ $outlet->trashed() ? 'bg-gray-50/50' : '' }}">
                                <td class="px-6 py-4 {{ $outlet->trashed() ? 'opacity-60' : '' }}">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-12 h-12 rounded-xl {{ $outlet->trashed() ? 'bg-gray-200 text-gray-500' : 'bg-orange-50 text-[#CC9863]' }} flex items-center justify-center font-bold shrink-0">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4">
                                                </path>
                                ...
                                <td class="px-6 py-4 {{ $outlet->trashed() ? 'opacity-60' : '' }}">
                                    <div class="flex items-center gap-2">
                                        <span class="font-bold text-gray-900">{{ $outlet->users_count }}</span>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function getFilterUrl() {
                const url = new URL('{{ route('owner.outlet.index') }}');
