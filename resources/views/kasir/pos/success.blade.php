...
@section('content')
    ...
    <div id="non-print-area"
        class="flex-1 overflow-y-auto p-4 lg:p-8 bg-[#FAFAFA] w-full flex items-center justify-center h-full">
        <div class="bg-white rounded-3xl border border-gray-100 shadow-xl w-full max-w-lg overflow-hidden relative">
            ...
            <div class="p-8 md:p-10 relative z-10 flex flex-col items-center text-center">
                ...
                <div
                    class="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mb-6 shadow-sm ring-8 ring-white relative">
                ...
                <p class="text-gray-500 text-sm mb-8">No. Invoice: <span
                        class="font-bold text-gray-700">{{ $transaction->nomor_nota }}</span></p>
                ...
                <div class="w-full bg-gray-50 rounded-2xl p-5 mb-8 border border-gray-100">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <span
                            class="text-sm font-bold text-gray-900 uppercase">{{ $transaction->metode_bayar === 'cash_tempo' ? 'TEMPO' : str_replace('_', ' ', $transaction->metode_bayar) }}</span>
                    </div>
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-sm font-semibold text-gray-500">Total Tagihan</span>
                        <span class="text-sm font-bold text-gray-900">Rp
                            {{ number_format($transaction->total_belanja, 0, ',', '.') }}</span>
                    </div>
                    ...
                    @if ($transaction->metode_bayar === 'cash')
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <span class="text-sm font-bold text-gray-900">Rp
                                {{ number_format($transaction->nominal_bayar, 0, ',', '.') }}</span>
                        </div>
                    @endif
                    ...
                    @if ($transaction->cashTempo)
                        <div class="mb-4 rounded-xl border border-red-100 bg-red-50/70 p-3">
                            ...
                            <div class="flex justify-between items-center mt-2 text-sm">
                                ...
                                <span
                                    class="font-bold text-gray-900">{{ \Carbon\Carbon::parse($transaction->cashTempo->tanggal_jatuh_tempo)->translatedFormat('d M Y') }}</span>
                            </div>
                            <div class="flex justify-between items-center mt-2 text-sm">
                                <span class="font-semibold text-red-700">Sisa Piutang</span>
                                <span class="font-bold text-red-700">Rp
                                    {{ number_format($transaction->cashTempo->sisa_piutang, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div
                        class="flex justify-between items-center {{ $transaction->cashTempo && $transaction->cashTempo->sisa_piutang > 0 ? 'bg-red-50/50 border-red-100 text-red-600' : 'bg-green-50/50 border-green-100 text-green-600' }} p-3 rounded-xl border">
                        <span class="text-sm font-bold">{{ $transaction->cashTempo ? 'Sisa Piutang' : 'Kembalian' }}</span>
                        <span class="text-2xl font-extrabold">Rp
                            {{ number_format($transaction->cashTempo ? $transaction->cashTempo->sisa_piutang : $transaction->kembalian, 0, ',', '.') }}</span>
                    </div>
                </div>
                ...
                <div class="w-full space-y-3">
                    ...
                    <button onclick="window.print()"
                        class="w-full bg-[#1C1D21] text-white py-4 rounded-xl font-bold text-base hover:bg-gray-800 transition shadow-lg flex justify-center items-center gap-2">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        @php
                            ...
                            $phone = $transaction->member
                                ? preg_replace('/^0/', '62', $transaction->member->no_telp)
                                : '';
                            ...
                            $waText .=
                                '📅 *Tanggal:* ' .
                                \Carbon\Carbon::parse($transaction->tanggal_waktu)->format('d M Y H:i') .
                                "\n";
                            $waText .=
                                '💳 *Metode:* ' . strtoupper(str_replace('_', ' ', $transaction->metode_bayar)) . "\n";
                            $waText .=
                                '🛒 *Total Tagihan:* Rp ' .
                                number_format($transaction->total_belanja, 0, ',', '.') .
                                "\n";

                            if ($transaction->metode_bayar === 'cash') {
                                $waText .=
                                    '💵 *Uang Diterima:* Rp ' .
                                    number_format($transaction->nominal_bayar, 0, ',', '.') .
                                    "\n";
                                $waText .=
                                    '🔄 *Kembalian:* Rp ' . number_format($transaction->kembalian, 0, ',', '.') . "\n";
                            } elseif ($transaction->cashTempo && $transaction->cashTempo->sisa_piutang > 0) {
                                $waText .=
                                    '💵 *Pembayaran Awal:* Rp ' .
                                    number_format($transaction->nominal_bayar, 0, ',', '.') .
                                    "\n";
                                $waText .=
                                    '⏳ *Sisa Piutang:* Rp ' .
                                    number_format($transaction->cashTempo->sisa_piutang, 0, ',', '.') .
                                    "\n";
                                $waText .=
                                    '📅 *Jatuh Tempo:* ' .
                                    \Carbon\Carbon::parse(
                                        $transaction->cashTempo->tanggal_jatuh_tempo,
                        ...
                        <a href="{{ $waLink }}" target="_blank"
                            class="w-full {{ $phone ? 'bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20' : 'bg-gray-100 text-gray-400 cursor-not-allowed' }} border border-transparent py-3.5 rounded-xl font-bold transition flex justify-center items-center gap-2 text-sm"
                            {!! !$phone ? 'onclick="event.preventDefault(); AppFeedback.warning(\'Nomor pelanggan tidak tersedia\');"' : '' !!}>
                        ...
                        <a href="{{ route('kasir.pos') }}"
                            class="w-full bg-[#CC9863] text-white py-3.5 rounded-xl font-bold hover:bg-[#b58555] transition flex justify-center items-center text-sm shadow-sm">
                            Transaksi Baru
                        </a>
                    </div>
                </div>
    ...
    <div id="receipt-area" class="hidden text-black font-mono text-[12px] leading-tight">
        ...
        <div style="text-align: center; margin-bottom: 10px;">
            ...
            <p style="margin: 2px 0;">{{ $transaction->branch->alamat ?? 'Jl. Contoh Alamat Toko No. 123' }}</p>
            <p style="margin: 2px 0;">Telp: {{ $transaction->branch->no_telp ?? '0812-3456-7890' }}</p>
        </div>
        ...
        <div style="border-bottom: 1px dashed #000; margin-bottom: 8px; padding-bottom: 8px;">
            ...
            <p style="margin: 2px 0;">Tgl : {{ \Carbon\Carbon::parse($transaction->tanggal_waktu)->format('d/m/Y H:i') }}
            </p>
                ...
                @foreach ($transaction->details as $item)
                    ...
                    <div style="margin-bottom: 6px;">
                        <p style="margin: 0; font-weight: bold;">
                            {{ $item->variant->product->nama_produk ?? 'Produk' }} -
                            {{ $item->variant->nama_varian ?? 'Item' }}
                        </p>
        ...
        <div style="border-bottom: 1px dashed #000; margin-bottom: 8px; padding-bottom: 8px;">
            <div style="display: flex; justify-content: space-between;">
                <span>Metode</span>
                <span
                    style="text-transform: uppercase;">{{ $transaction->metode_bayar === 'cash_tempo' ? 'TEMPO' : str_replace('_', ' ', $transaction->metode_bayar) }}</span>
            </div>
            @if ($transaction->metode_bayar === 'cash')
                <div style="display: flex; justify-content: space-between;">
        ...
        <div style="text-align: center; margin-top: 10px;">
            ...
            <p style="margin: 4px 0 2px; font-size: 10px;">Barang yang sudah dibeli</p>
    ...
    <style>
        ...
        @media print {
            ...
            #receipt-area {
                ...
                top: 0;
                width: 58mm;
                /* Lebar default printer kasir kecil */
                padding: 5mm;
